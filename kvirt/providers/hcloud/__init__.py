            ...
            try:
                ...
                if e.code == "resource_unavailable":
                    if len(flavor_options) > (idx + 1):
                        warning(f"Could not get server of type '{flavor_option}' in location '{self.location.name}' at current time, trying next flavor option")
                    else:
                        warning(f"Could not get server of type '{flavor_option}' in location '{self.location.name}' at current time")
                        error(f"No more flavors available to try. Define more flavors in 'flavor_options' to have fallback options")
